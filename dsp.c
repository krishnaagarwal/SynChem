...
static void adjust_piece (Dsp_Atom_t *, int, int, Boolean_t *);
static void visit_nbr1 (Dsp_Atom_t *, int, Boolean_t *);
static void visit_nbr2 (Dsp_Atom_t *, int, Boolean_t *, int *, int *, int *, int *, int);
		   ...
		   Dsp_Atom_t      *atom_p)
{
  ...
  /* Find number of this atom in "mol_p" structure */
...
{//kka
  ...
  for (atm_i = 0; atm_i < atm_num; atm_i++) 
    {
    for (ngb_i = 0; ngb_i < atom_p->adj_info.num_neighbs; ngb_i++)
      adj_list[atm_i][ngb_i] = 0;//kka:atom_p->adj_info.adj_atms[ngb_i] + 1;
    xarray[atm_i]=0;
    yarray[atm_i]=0;
    if (atm_i==0)
    { 
/*kka      strcpy (atom_p->sym, "Err");
      strcpy (atom_p->chg, "\0");
      strcpy (atom_p->map, "\0");
      if (strcmp (atom_p->sym, "C") == 0)
        atom_p->isC = TRUE;
      else
        atom_p->isC = FALSE; */
      xarray[atm_i]=1;
...
//  draw2_ (&adj_list[0][0], &atm_num, xarray, yarray);
  ...
  /* Convert cartesian xy-coordinates into Motif coordinates.
...
/********************************************
**       Dsp_Compress (adjusts multiatomic pieces leftward to overlay (invisible) monatomic pieces
*********************************************/
void Dsp_Compress (Dsp_Molecule_t *dsp)
{
  Boolean_t *visited, change;
  int i, j, k, temp, npieces, piece, *minx, *miny, *maxx, *maxy, *first_atom, *piece_map, x_adjust;

  visited = (Boolean_t *) malloc (dsp->natms * sizeof (Boolean_t));

  for (i = 0; i < dsp->natms; i++) visited[i] = FALSE;

  for (i = npieces = 0; i < dsp->natms; i++) if (!visited[i])
    {
    npieces++;
    visit_nbr1 (dsp->atoms, i, visited);
    }

  minx = (int *) malloc ((npieces + 1) * sizeof (int));
  miny = (int *) malloc ((npieces + 1) * sizeof (int));
  maxx = (int *) malloc ((npieces + 1) * sizeof (int));
  maxy = (int *) malloc ((npieces + 1) * sizeof (int));
  first_atom = (int *) malloc ((npieces + 1) * sizeof (int));
  piece_map = (int *) malloc ((npieces + 1) * sizeof (int));

  for (i = 0; i < dsp->natms; i++) visited[i] = FALSE;

  for (i = 0; i <= npieces; i++) piece_map[i] = i;

  minx[0] = miny[0] = 100000;
  maxx[0] = maxy[0] = 0;

  for (i = piece = 0; i < dsp->natms; i++) if (!visited[i])
    {
    ++piece;
    first_atom [piece] = i;
    minx[piece] = miny[piece] = 100000;
    maxx[piece] = maxy[piece] = 0;
    visit_nbr2 (dsp->atoms, i, visited, minx, miny, maxx, maxy, piece);
    }

  for (change = TRUE; change;)
    for (i = 1, change = FALSE; i < npieces; i++)
      for (j = i + 1; j <= npieces; j++) if (minx[piece_map[i]] > minx[piece_map[j]])
    {
    change = TRUE;

    temp = piece_map[i];
    piece_map[i] = piece_map[j];
    piece_map[j] = temp;
    }
  ...
  for (piece = 1; piece < npieces; piece++)
    {
    for (i = 0; i < dsp->natms; i++) visited[i] = FALSE;
    j = first_atom[piece_map[piece]];
    if (dsp->atoms[j].adj_info.num_neighbs == 0)
      {
      x_adjust = minx[piece_map[piece]] - minx[piece_map[piece + 1]];
      for (k = piece + 1; k <= npieces; k++)
        {
        adjust_piece (dsp->atoms, first_atom[piece_map[k]], x_adjust, visited);
        minx[piece_map[k]] += x_adjust;
        maxx[piece_map[k]] += x_adjust;
        }
      dsp->molw += x_adjust;
      }
    }
  maxx[0] = maxx[piece_map[npieces]];
...
static void adjust_piece (Dsp_Atom_t *atoms, int ainx, int x_adjust, Boolean_t *visit)
{
  int i, ninx;

  visit[ainx] = TRUE;
  atoms[ainx].x += x_adjust;

  for (i = 0; i < atoms[ainx].adj_info.num_neighbs; i++)
    {
    ninx = atoms[ainx].adj_info.adj_atms[i];
    if (!visit[ninx]) adjust_piece (atoms, ninx, x_adjust, visit);
    }
}

static void visit_nbr1 (Dsp_Atom_t *atoms, int ainx, Boolean_t *visit)
{
  int i, ninx;
  ...
  for (i = 0; i < atoms[ainx].adj_info.num_neighbs; i++)
    {
    ninx = atoms[ainx].adj_info.adj_atms[i];
    if (!visit[ninx]) visit_nbr1 (atoms, ninx, visit);
    }
}

static void visit_nbr2 (Dsp_Atom_t *atoms, int ainx, Boolean_t *visit, int *minx, int *miny, int *maxx, int *maxy, int piece)
{
  int i, ninx, x, y;
  ...
  for (i = 0; i < atoms[ainx].adj_info.num_neighbs; i++)
    {
    ninx = atoms[ainx].adj_info.adj_atms[i];
    if (!visit[ninx]) visit_nbr2 (atoms, ninx, visit, minx, miny, maxx, maxy, piece);
    }
}
