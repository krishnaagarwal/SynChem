...
int *hash[256], hashcount[256], hashalloc[256];
...
int hash_value (Str arg, int include_enc)
{
  ...
  for (i=0; i<arg.len; i++) xor[i%4]^=(arg.val[i]&3);
  if (include_enc) xor[i%4]^=(EncChar&3);
  i = xor[0] | (xor[1]<<2) | (xor[2]<<4) | (xor[3]<<6);
