...
* 16-May-02  Miller     "Backed in" SBULK changed, but with corrected conditional logic.
*                       This now replicates all the original thesis results.
*
******************************************************************************/
...
#ifdef _SOLARIS_OS_
/* NOTE: This is merely a temporary hack.  The non-portability of the
   Zadehan_t union between little- and big-Endian machines should be
   addressed in a more transparent manner, and the typedef should be
   moved into a header file for consistency. */
typedef union
{
  U8_t fuzzy_value;
  struct
  ...
  )
{
  ...
  for ( i = num_hnbrs = 0, last_son = NULL; i < tree_node_p-> num_of_neighbors; i++)
    ...
    if (neighbor != father && neighbor != origin_node 
      && neighbor != origin_father)
      {
      if (which_case && Xtr_Attr_Atomid_Get (xtr_p, neighbor) == 1) num_hnbrs++; /* GAM 05/16/02 */
      else
        {
...
static int fuzzy_strcmp (U8_t *str1, U8_t *str2)
{
  ...
  for (s1 = (char *) str1, s2 = (char *) str2, pos = 0; *s1 && *s1 == *s2; s1++, s2++, pos++);
...
{
#ifdef _FDEBUG_
 printf("fuzzy_strcmp=0\n");
#endif
 return(0);
}
diff=c1>c2?1:-1;
/*diff = (5 * (c1 - c2) / pos + 5 * (c1 - c2)) / 10; /* This is a very crude heuristic! */
diff=20*diff/pos+diff;
#ifdef _FDEBUG_
printf("fuzzy_strcmp=%d\n",diff);
#endif
return(diff);
}
...
static Zadehan_t Zadehan_Normalized (Zadehan_t in_fuzzy)
{
  ...
  out_fuzzy = in_fuzzy;
/* NOTE: The following is temporarily retained, despite the hacklike
   nature of its use of the fuzzy_value byte value.  The non-portability
   of the Zadehan_t union between little- and big-Endian machines should
   be addressed in a more transparent manner. */
