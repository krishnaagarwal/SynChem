...
void Pre_Form_Create (Widget top_level, U32_t schn, U32_t *roots, U32_t *synthemes)
{
	Boolean_t
		...
		anycl, anycn, anyml, anymn, initial;
	XmFontListEntry
		courb18, helv18;
	XmFontList
		flcob18, flhv18;
...
else printf("t_f: did not deselect %d\n", i);
        }
printf("t_f3: %d\n", XmListPosSelected(w,4));
	toggle_on=TRUE;
        XtFree ((char *) pos);
...
void fg_deselect_all(Widget w,Boolean_t initial)
{
	...
	if (initial) for (j=0; j<2; j++)
		...
		{
/* First get rid of any leftovers from previous display! */
		for (i=1; i<=NUM_TEMPLATES; i++) if (fgpossel[1][3][i]) fg_pos_deselect (w, i, FALSE);
		else if (XmListPosSelected (w, i)) XmListDeselectPos (w, i);
		for (i=pos=1; i<MX_FUNCGROUPS; i++) if (fgsel[1][3][i]) fg_deselect(w,i);
		else if (fgname[i] != NULL)
		{
			if (XmListPosSelected (w, pos+NUM_TEMPLATES)) XmListDeselectPos (w, pos+NUM_TEMPLATES);
			pos++;
		}
/* Then do this! */
		for (j=0; j<2; j++)
...
printf("f_p_s7: %d\n",XmListPosSelected(w,4));
	XmListSelectPos(w,pos+NUM_TEMPLATES,FALSE);
printf("f_p_s8 (%d): %d\n",pos+NUM_TEMPLATES,XmListPosSelected(w,4));
}
