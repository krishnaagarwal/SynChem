...
* 21-Dec-01  Miller     Added fuzziness to fgroup_penalty calculations that use
*                       the Pairs array.
* 04-Jan-02  Miller     Added Alkylation_Strategic to reduce the Carbon_Table
*                       penalty by half for ternary and quaternary carbons for
*                       which the neighboring groups tend to facilitate an
*                       alkylative construction (i.e., dicarbonyl or carbonyl
*                       and electronegative heteroatom)
******************************************************************************/
...
static Boolean_t   Alkylation_Strategic    (Xtr_t *, U16_t);
...
static U16_t Pairs[47][3] = 
  {
    ...
    {41, 452,  75},   /* P-YLID */
    /* ALDEHYDE AND... */           
    {50,  68,  90},   /* VINYL ETHER */
    {50,  69,  90},   /* ACETAL */
    {50,  70,  90},   /* KETAL */
    {50,  74, 100},   /* ORTHOESTER */
    {50, 117,  95},   /* HYDRAZONE */
    {50, 119, 100},   /* DIAZO */
    {50, 123, 100},   /* PRIMARY AMINE */
    {50, 124, 100},   /* SECONDARY AMINE */
    {50, 136, 100},   /* MERCAPTAN */
    {50, 353, 100},   /* AMIDINE */
    {50, 356,  95},   /* AMINAL */
    {50, 452, 100},   /* P-YLID */
    {50, 475, 100},   /* SELENOL */
    /* KETONE AND... */             
    {51, 117,  90},   /* HYDRAZONE */
    {51, 123, 100},   /* PRIMARY AMINE */
    {51, 475, 100},   /* SELENOL */
    /* EPOXIDE AND... */            
    {65,   5, 100},   /* RCOOH */
    {65,  79, 100},   /* ANHYDRIDE */
    {65, 114, 100},   /* DIAZONIUM SALT */
    {65, 135,  75},   /* SULFIDE */
    {65, 136, 100},   /* MERCAPTAN */
    {65, 315, 100},   /* EPISULFIDE */
    {65, 350, 100},   /* AZIRIDINE */
    {65, 450, 100},   /* PHOSPHINE */
    {65, 475, 100},   /* SELENOL */
    {65, 479,  80},   /* SELENIDE */
    /* ANHYDRIDE AND... */          
    {79,   1, 100},   /* HYDROXYL */
    {79,  14,  50},   /* PYRIDINE, ETC. */
    {79, 126, 100},   /* OXIME */
    {79, 134,  75},   /* SULFOXIDE */
    {79, 136,  95},   /* MERCAPTAN */
    {79, 137,  50},   /* SULFONIC ACID */
    {79, 475, 100},   /* SELENOL */
    /* ACID HALIDES AND... */       
    {85,   1, 100},   /* HYDROXYL */
    {85, 126, 100},   /* OXIME */
    {85, 134,  80},   /* SULFOXIDE */
    {85, 136, 100},   /* MERCAPTAN */
    {85, 475, 100},   /* SELENOL */
    /* EPISULFIDE AND... */         
    {315, 350, 100},   /* AZIRIDINE */
    {315, 450, 100}    /* PHOSPHINE */
  };
...
#ifdef _DEBUG_
printf("rings_penalty=%d carbons_penalty=%d fgroups_penalty=%d\n",rings_penalty,carbons_penalty,fgroups_penalty);
#endif
...
U16_t strategic_bonus;
  ...
  for (i = 1; i <= num_carbons; ++i)
	...
	sum += (300 / factor);
        ...
        {
        if (factor == 1 && j < 3 && Array_1d16_Get (&hydrogen, atom) < 2 && Array_1d16_Get (&sigma, atom) > 2)
        /* ternary or quaternary acyclic or non-aromatic ring compounds */
          {
#ifdef _DEBUG_
printf("calling Alkylation_Strategic for atom %d\n", atom);
#endif
          if (Alkylation_Strategic (xtr_p, atom)) factor = 3 /*strategic_bonus = 60*/;
          }
	sum += ((Carbon_Table [j - 1][Array_1d16_Get (&hydrogen, atom)]
		[Array_1d16_Get (&sigma, atom) - 1] - strategic_bonus) / factor);
#ifdef _DEBUG_
printf("atom=%d strategic_bonus=%d j=%d h=%d s=%d sum=%d\n",atom,strategic_bonus,j,(int)(U16_t)Array_1d16_Get(&hydrogen,atom),(int)(U16_t)Array_1d16_Get(&sigma,atom),sum);
#endif
        }
      }
    }
...
/****************************************************************************
*
*  Function Name:                 Alkylation_Strategic
*
****************************************************************************/
static Boolean_t Alkylation_Strategic
  (
  Xtr_t        *xtr_p,
  U16_t         atom
  )
{
  int i, j, nbr, atomid, ncarbonyl, nhetero, nbr_nbr_bond, nbr_nbr, nbr_nbr_id, noxy_nbr;

  for (i = ncarbonyl = nhetero = 0; i < 4 && (ncarbonyl == 0 || ncarbonyl + nhetero < 2); i++)
    {
    nbr = Xtr_Attr_NeighborId_Get (xtr_p, atom, i);
    atomid = Xtr_Attr_Atomid_Get (xtr_p, nbr);
#ifdef _DEBUG_
printf("nbr=%d atomid=%d\n",nbr,atomid);
#endif
    switch (atomid)
      {
    case OXYGEN:
    ...
    case SULFUR:
    case SELENIUM:
      /* treat oxidized forms in the same way as carbonyl/nitrile activation */
      for (j = noxy_nbr = 0; j < Xtr_Attr_NumNeighbors_Get (xtr_p, nbr) && noxy_nbr == 0; j++)
        {
        nbr_nbr_bond = Xtr_Attr_NeighborBond_Get (xtr_p, nbr, j);
        nbr_nbr = Xtr_Attr_NeighborId_Get (xtr_p, nbr, j);
        nbr_nbr_id = Xtr_Attr_Atomid_Get (xtr_p, nbr_nbr);
        if (nbr_nbr_bond == BOND_DOUBLE && nbr_nbr_id == OXYGEN) noxy_nbr++;
        }
      if (noxy_nbr > 0) ncarbonyl++;
      else nhetero++;
      break;
    case CARBON:
      for (j = 0; j < Xtr_Attr_NumNeighbors_Get (xtr_p, nbr); j++)
        if (Xtr_Attr_NeighborBond_Get (xtr_p, nbr, j) == BOND_RESONANT)
        /* treat benzylic activation in the same way as heterocyclic activation */
        {
        nhetero++;
        break;
        }
        ...
        nbr_nbr = Xtr_Attr_NeighborId_Get (xtr_p, nbr, j);
        nbr_nbr_id = Xtr_Attr_Atomid_Get (xtr_p, nbr_nbr);
        if ((nbr_nbr_bond == BOND_DOUBLE && nbr_nbr_id == OXYGEN) ||
          (nbr_nbr_bond == BOND_TRIPLE && nbr_nbr_id == NITROGEN))
          {
          ncarbonyl++;
          break;
          }
...
#ifdef _DEBUG_
printf("ncarbonyl=%d nhetero=%d\n",ncarbonyl,nhetero);
#endif
  return (ncarbonyl > 0 && ncarbonyl + nhetero > 1);
}
...
if (flag) fuzzy_factor = 100;
else fuzzy_factor = 0;
 
  for (i = 0; i < 47 && (flag == FALSE || fuzzy_factor < 100); ++i)
    {
    flag = (Xtr_FuncGrp_NumInstances_Get (xtr_p, Pairs[i][0]) > 0 &&
	    Xtr_FuncGrp_NumInstances_Get (xtr_p, Pairs[i][1]) > 0);
    if (Pairs[i][2] > fuzzy_factor) fuzzy_factor = Pairs[i][2];
    }
 
  if (flag == TRUE)
    return 35 * fuzzy_factor / 100;
  else
    return 0;
}
...
printf ("after loop: sum = %d\n", sum);
  ...
  if (ratio > 5)               /* too many functional groups */
    sum = 10 * (ratio - 5);
#ifdef _DEBUG_
printf ("after ratio: ratio = %d, sum = %d\n", ratio, sum);
#endif
  ...
  sum += 5 * metastable;
#ifdef _DEBUG_
printf ("before Pst_FGroupPenalty_Find: sum = %d\n", sum);
#endif
  sum += Pst_FGroupPenalty_Find (xtr_p);
#ifdef _DEBUG_
printf ("after Pst_FGroupPenalty_Find: sum = %d\n", sum);
#endif
#ifdef _MIND_MEM_
