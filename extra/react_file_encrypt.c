...
Tsd_t *unenctsd;
  ...
  if (text_p != NULL)
    {
    thead_p = React_TxtRec_Head_Get (text_p);
    unencrypted = String_Encrypt (React_TxtRec_Name_Get (text_p), 0, React_Head_SchemaId_Get (head_p));
    fprintf (f, "Reaction name is %s\n", String_Value_Get (unencrypted));
...
/*
  fprintf (f, "\n\nGoal TSD\n");
  unenctsd = Tsd_Encrypt (React_Goal_Get (react_p), 0, React_Head_SchemaId_Get (head_p));
  Tsd_Dump (unenctsd, filed_p);
  Tsd_Destroy (unenctsd);
  fprintf (f, "\nSubgoal TSD\n");
  unenctsd = Tsd_Encrypt (React_Subgoal_Get (react_p), 1, React_Head_SchemaId_Get (head_p));
  Tsd_Dump (unenctsd, filed_p);
  Tsd_Destroy (unenctsd);
...
*/
  ...
  for (i = 0; i < React_Head_NumTests_Get (head_p); i++)
    ...
    if (text_p != NULL)
      ...
      unencrypted = String_Encrypt (React_TxtRec_Reason_Get (text_p, i), i, React_Head_SchemaId_Get (head_p));
      fprintf (f, "Rationale for test  :\t%s\n", String_Value_Get (unencrypted));
      String_Destroy (unencrypted);
      unencrypted = String_Encrypt (React_TxtRec_Chemist_Get (text_p, i), i, React_Head_SchemaId_Get (head_p));
      fprintf (f, "Chemist\t\t    :\t%s\n\n", String_Value_Get (unencrypted));
      String_Destroy (unencrypted);
      }
    }
  ...
  if (text_p != NULL)
    ...
    for (i = 0; i < React_TxtHd_NumComments_Get (thead_p); i++)
      {
      unencrypted = String_Encrypt (React_TxtRec_Comment_Get (text_p, i), i, React_Head_SchemaId_Get (head_p));
      fprintf (f, "%s\n", String_Value_Get (unencrypted));
      String_Destroy (unencrypted);
      }
    ...
    for (i = 0; i < React_TxtHd_NumReferences_Get (thead_p); i++)
      {
      unencrypted = String_Encrypt (React_TxtRec_Reference_Get (text_p, i), i, React_Head_SchemaId_Get (head_p));
      fprintf (f, "%s\n", String_Value_Get (unencrypted));
      String_Destroy (unencrypted);
      }
    }
