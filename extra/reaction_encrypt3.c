...
unenctsd = Tsd_Encrypt (React_Goal_Get (SReactions[destination]), 0, source);
...
unenctsd = Tsd_Encrypt (React_Subgoal_Get (SReactions[destination]), 1, source);
...
String_t unencrypted;
  ...
  goal = Tsd_Encrypt (React_Goal_Get (schema), 0, source);
  subgoal = Tsd_Encrypt (React_Subgoal_Get (schema), 1, source);
  ...
  unencrypted = String_Encrypt (React_TxtRec_Name_Get (text), 0, source);
  ...
  for (i = 0; i < React_TxtHd_NumComments_Get (txt_head); i++)
    {
    strcat (buffer, "\t");
    unencrypted = String_Encrypt (React_TxtRec_Comment_Get (text, i), i, source);
    Sprintf (line, unencrypted);
    String_Destroy (unencrypted);
    strcat (buffer, line);
    }
  ...
  for (i = 0; i < React_TxtHd_NumReferences_Get (txt_head); i++)
    {
    strcat (buffer, "\t");
    unencrypted = String_Encrypt (React_TxtRec_Reference_Get (text, i), i, source);
    Sprintf (line, unencrypted);
    String_Destroy (unencrypted);
    strcat (buffer, line);
    }
  ...
  for (i = 0; i < React_Head_NumTests_Get (sch_head); i++)
    ...
    strcat (buffer, line);
    strcat (buffer, "\t\t");
    unencrypted = String_Encrypt (React_TxtRec_Reason_Get (text, i), i, source);
    Sprintf (line, unencrypted);
    String_Destroy (unencrypted);
    strcat (buffer, line);
    strcat (buffer, "\t\t");
    unencrypted = String_Encrypt (React_TxtRec_Chemist_Get (text, i), i, source);
    Sprintf (line, unencrypted);
    String_Destroy (unencrypted);
    strcat (buffer, line);
    }
