...
main ()
{
	...
	strcpy (IO_FileName_Get (Isam_File_Get (&SDataCB)), "//E/SYNCHEM/testdata/rxnlib/rkbdata.isam");
	strcpy (IO_FileName_Get (Isam_File_Get (&STextCB)), "//E/SYNCHEM/testdata/rxnlib/rkbtext.isam");
	Isam_Open (&SDataCB, ISAM_TYPE_RKBDATA, ISAM_OPEN_INIT);
	Isam_Close (&SDataCB);
	Isam_Open (&STextCB, ISAM_TYPE_RKBTEXT, ISAM_OPEN_INIT);
	Isam_Close (&STextCB);
	pf=fopen("//E/SYNCHEM/testdata/rxnlib/rkbstd.inx","w");
	...
	fclose(pf);
	React_Init ("R+W//E/SYNCHEM/testdata/rxnlib");
	for (j = done = 0; !done; j++)
		{
		b = buf;
		do
