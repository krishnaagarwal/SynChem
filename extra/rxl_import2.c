...
main ()
{
	...
	char buf[100000], line[1024], *b, *l, junk[4][256];
	Isam_Control_t SDataCB;
	Isam_Control_t STextCB;

	strcpy (IO_FileName_Get (Isam_File_Get (&SDataCB)), FCB_SEQDIR_RXNS ("/rkbdata.isam"));
	strcpy (IO_FileName_Get (Isam_File_Get (&STextCB)), FCB_SEQDIR_RXNS ("/rkbtext.isam"));
	Isam_Open (&SDataCB, ISAM_TYPE_RKBDATA, ISAM_OPEN_INIT);
	Isam_Close (&SDataCB);
	Isam_Open (&STextCB, ISAM_TYPE_RKBTEXT, ISAM_OPEN_INIT);
	Isam_Close (&STextCB);
	pf=fopen(FCB_SEQDIR_RXNS ("/rkbstd.inx"),"w");
	...
	fclose(pf);
	React_Init (concat ("R+W", FCB_SEQDIR_RXNS ("")));
	for (j = done = 0; !done; j++)
		{
printf("j=%d\n",j);
		b = buf;
