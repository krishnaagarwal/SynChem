...
dump_RxnDsp(rxndsp_p, "start of rxndsp_Shelley\n");
  if (rxndsp_p->goal_dm_p == NULL)
    {
...
dump_RxnDsp(rxndsp_p, "end of rxndsp_Shelley\n");
  return (TRUE);
}
...
RxnDsp_Molecule_t *Xtr2RxnDsp (Xtr_t *goalxtr, Xtr_t *subgxtr, U32_t *roots,
  U32_t *synthemes)
{
  static RxnDsp_Molecule_t rxnpatt;
...
printf("root %d (%d, %d): %d\n",nroots,RxnDsp_RootPtr_Get (&rxnpatt, i)->x,
RxnDsp_RootPtr_Get (&rxnpatt, i)->y,RxnDsp_RootPtr_Get (&rxnpatt, i)->syntheme);
    }
