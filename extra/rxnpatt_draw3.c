  ...
  )
{
  ...
  Display          *display;
  Widget            form;
  XmString          lbl_drw, lbl_sel, lbl_srt, lbl_del, lbl_drt;
...
printf("point 17\n");
  ...
  XtVaSetValues (PDrawTool_RedrawPB_Get (&GPDrawToolCB),
    XmNtopOffset,        0,
    XmNtopAttachment,    XmATTACH_OPPOSITE_WIDGET,
    XmNtopWidget,        PDrawTool_DonePB_Get (&GPDrawToolCB),
    XmNbottomOffset,     0,
    XmNbottomAttachment, XmATTACH_OPPOSITE_WIDGET,
    XmNbottomWidget,     PDrawTool_DonePB_Get (&GPDrawToolCB),
    XmNleftOffset,       0,
    XmNleftAttachment,   XmATTACH_POSITION,
    XmNleftPosition,     64,
    XmNrightOffset,      0,
...
printf("out of PDraw_Mode_Reset\n");  
  return;
...
printf("into SynList_Create\n");
  ...
  while((fgrec = FuncGrp_Rec_Read (fgrec_num, fgfile)) != NULL
    ...
    for (name_inx = 0;
      ...
      XmStringFree (lbl_str);
printf("synlist[%d]=%d (array); %d (macro)\n",fglist_pos-1,fg_num,
PDrawTool_SynListNum_Get (&GPDrawToolCB, fglist_pos));
      if (name_inx == 0)
        {
        sling =
          (char *) malloc
...
printf("into PPeriodic_TblDlg_Create\n");
...
printf("out of PPeriodic_TblDlg_Create\n");
  return;
}
...
printf("into PPixMap_Display_CB\n");
...
printf("out of PPixMap_Display_CB\n");
   return;
...
{ 
printf("into PPixMap_Resize_CB\n");
  if (XtIsRealized (w))
    XClearArea (XtDisplay(w), XtWindow(w), 0, 0, 0, 0, TRUE);
 
printf("out of PPixMap_Resize_CB\n");
 return;
...
printf("out of PCreateOptionMenu\n");
  return;
} 
...
printf("into PDraw_Tool_Initialize\n");
...
else printf("PDrawMol_p not NULL\n");
...
printf("out of PDraw_Tool_Initialize\n");
  return ;
}
...
printf("into PDraw_Draw: %d\n",PDrawFlags.drawing_mode);
...
printf("adj_j=%d adj_k=%d\n",adj_j,adj_k);
	    ...
	    if (!rxn_delete_Bond (PDrawMol_p, drbnd_p, gsgsame ? BOTH : gsg))
              ...
              {
printf("before rxn_delete_BOND: SUBG=%d\n",SUBG);
	      pdraw_Atom (PDrawDisplay, 
                XtWindow (PDrawTool_DrawArea_Get (&GPDrawToolCB, GOAL)),
                XtWindow (PDrawTool_DrawArea_Get (&GPDrawToolCB, SUBG)), gc, 
	      ...
	      if (!rxn_delete_Atom (PDrawMol_p, atom_p, gsgsame ? BOTH : gsg))
                {
printf("after rxn_delete_Bond: SUBG=%d\n",SUBG);
	        pdraw_Atom (PDrawDisplay, 
                  XtWindow (PDrawTool_DrawArea_Get (&GPDrawToolCB, GOAL)),
                  XtWindow (PDrawTool_DrawArea_Get (&GPDrawToolCB, SUBG)), gc, 
...
printf("after all adj bonds: SUBG=%d\n",SUBG);
	  pdraw_Atom (PDrawDisplay, 
            XtWindow (PDrawTool_DrawArea_Get (&GPDrawToolCB, GOAL)),
            XtWindow (PDrawTool_DrawArea_Get (&GPDrawToolCB, SUBG)), gc, 
...
printf("PA_M_C\n");
  ...
  if (!(dratm_p = RxnDsp_AtomThere_Get (PDrawMol_p, PDrawBondsInfo->clicked_gsg,
       PDrawBondsInfo->clicked_x, PDrawBondsInfo->clicked_y)))
{
printf("Couldn't find (%d, %d) in %s pattern\n",
PDrawBondsInfo->clicked_x, PDrawBondsInfo->clicked_y,
PDrawBondsInfo->clicked_gsg?"subgoal":"goal");
getchar();
    return;
}
    ...
    case PDRW_SELECTROOT_FLAG:
      if (selecting_root)
        ...
        if (!XtIsManaged (PDrawTool_SynListForm_Get (&GPDrawToolCB)) &&
          ...
          selecting_root = FALSE;
printf("Ignoring SELECTROOT button - selecting_root=%s\n",
selecting_root?"TRUE":"FALSE");
        break;
        }
...
printf("returning from POption_Choose_CB\n");
...
*/
        ...
        for (ii = 0; ii < PDrawMol_p->both_dm.natms; ii++) 
          {
printf("atom: pat=%d ii=%d\n", /*pat*/ PDrawSelInfo->selected_gsg,ii);
	  if (RxnDsp_AtomSelect_Get (atom_p, /* pat */
            PDrawSelInfo->selected_gsg) == DRW_SELECT_TOTAL) 
            {
printf("if select(3): SUBG=%d\n",SUBG);
	    pdraw_Atom (PDrawDisplay, 
...
*/
	    ...
	    if (!rxn_delete_Atom (PDrawMol_p, atom_p,
              gsgsame ? BOTH : PDrawSelInfo->selected_gsg))
              {
printf("!rxn_delete_Atom(2): SUBG=%d\n",SUBG);
	      pdraw_Atom (PDrawDisplay, 
                XtWindow (PDrawTool_DrawArea_Get (&GPDrawToolCB, /* start */
                PDrawSelInfo->selected_gsg)), 
                XtWindow (PDrawTool_DrawArea_Get (&GPDrawToolCB, /* finish */
...
printf("PMolDraw_Sling_Draw: sling = %s\n",(char *) Sling_Name_Get (sling));
...
printf("RD2T\n");
  ...
  for (atm_i = 0; atm_i < mol_p->both_dm.natms; atm_i++) 
    ...
    for (ii = 0; ii < atom_p->adj_info.num_neighbs; ii++) 
      ...
      bond_p = RxnDsp_BondThere_Get (mol_p, gsg,
        RxnDsp_AtomXCoord_Get (atom_p, gsg),
        RxnDsp_AtomYCoord_Get (atom_p, gsg), gsg,
        RxnDsp_AtomXCoord_Get (adjatm_p, gsg),
        RxnDsp_AtomYCoord_Get (adjatm_p, gsg));
...
printf("PM_D\n");
  ...
  free_Reaction (mol_p);
printf("exiting PMolecule_Double\n");  
...
*/
  ...
  if (mol_p->both_dm.natms == mol_p->both_dm.nallocatms) 
    ...
    PDrawMol_p = mol_p;
printf("pstore_AtomNew after PMolecule_Double: natms=%d (%d)\n",mol_p->both_dm.natms,mol_p->both_dm.nallocatms);
    }
...
printf("otherpatt_x = %d, otherpatt_y = %d\n",otherpatt_x,otherpatt_y);
...
printf("pd_B\n"); 
  ...
  if (RxnDsp_BondStereo_Get (bond_p, which) != DSP_BOND_STEREO_NONE)
    ...
    {
printf("case: which=%d BondOrder=\n",which);
printf("\t%d\n",RxnDsp_BondOrder_Get (bond_p, which));
    switch (RxnDsp_BondOrder_Get (bond_p, which)) 
      {
      case DSP_BOND_SINGLE:
...
printf("pd_A: onedrbl=%d gsg=%d GOAL=%d\n",onedrbl,gsg,GOAL);
/*
...
*/
  ...
  if (!dratm_p->isC) 
    {
printf("AtomSym: onedrbl=%s; drbl1=%d; drbl2=%d\n",onedrbl?"yes":"no",
(int)drbl1,(int)drbl2);
...
if (mol_p->both_dm.atoms[0].x != mol_p->goal_dm_p->atoms[0].x)
printf("pd_M1\n");
...
*/
for (k=0; k<mol_p->both_dm.natms; k++)
	{
	if (pat==0 && (l=root_syntheme(mol_p,k)))
		printf("\t*%d\t%2d: %s",l,k+1,
		Atomid2Symbol(Tsd_Atomid_Get(tsd_p,k)));
	else
		printf("\t\t%2d: %s",
		k+1,Atomid2Symbol(Tsd_Atomid_Get(tsd_p,k)));
	for (j=0; j<MX_NEIGHBORS; j++)
	{
		nbr=Tsd_Atom_NeighborId_Get(tsd_p,k,j);
		if (nbr!=TSD_INVALID)
		{
			bond=Tsd_Atom_NeighborBond_Get(tsd_p,
				k,j);
			if (bond!=BOND_NONE)
				printf("\t%2d:%d",nbr+1,bond);
		}
	}
	putchar('\n');
}
...
printf("entering draw_roots: %d roots\n",RxnDsp_RootCnt_Get (mol_p));
...
else printf("Root %d (%d, %d) != Atom %d (%d, %d)\n",i,
RxnDsp_RootPtr_Get(mol_p, i)->x,RxnDsp_RootPtr_Get(mol_p, i)->y,
j,RxnDsp_AtomXCoord_Get(RxnDsp_AtomPtr_Get(mol_p,j),GOAL),
RxnDsp_AtomYCoord_Get(RxnDsp_AtomPtr_Get(mol_p,j),GOAL));

printf("exiting draw_roots: %d roots\n",RxnDsp_RootCnt_Get (mol_p));
}
/* End of draw_roots */
...
printf("entering check_roots: %d roots\n",RxnDsp_RootCnt_Get (mol_p));
  for (i=0; i<RxnDsp_RootCnt_Get (mol_p); i++)
...
printf("exiting check_roots: %d roots\n",RxnDsp_RootCnt_Get (mol_p));
    return;
    }
}
/* End of check_roots */
...
printf("Syntheme_Store_CB entered\n");
printf("reason=%d; XmCR_SINGLE_SELECT=%d\n", cbs->reason, XmCR_SINGLE_SELECT);
...
*/
  XtUnmanageChild (PDrawTool_SynListForm_Get (&GPDrawToolCB));
printf("ss0\n");
printf("sling=%s\n",GPDrawToolCB.slings[cbs->item_position]);
...
printf("ss4\n");
    ...
    if (sling_roots[nslings][0] == '(')
      strncpy (sling_roots[nslings], sling_roots[nslings] + 1, 2);
...
printf("Selected FG: %d\n", Selected_Syntheme);
printf("exiting Syntheme_Store_CB\n");
}
