...
*/
extern Boolean_t postform_modified, imported_flag, was_modified[8];
...
void extract_flags (String_t);
void append_flags (String_t *);
...
void SchImport_CB (Widget, XtPointer, XtPointer);
void SchImport_Cont ();
void SchImport_Cancel_CB (Widget, XtPointer, XtPointer);
void SchImport_OK_CB (Widget, XtPointer, XtPointer);
Boolean_t schemata_compatible (int, int);
int names_similar (char *, char *);
void SchComp_Create_Form (Widget, XtAppContext, int *, React_Record_t **, Boolean_t *, void (*)());
int patterns_similar (Tsd_t *, Tsd_t *);
void Check_Import ();
...
static Boolean_t save_exit_flag = TRUE, pretests_modified = FALSE, posttests_modified = FALSE, transform_modified = FALSE,
  ...
  bypass_exit = FALSE, incflag_set = FALSE, filedg_initd = FALSE;
static Widget schhelppb, istherepb[2], left_brace_lbl, bypass_legend_lbl[2], bypass_toggle_pb, imppb, impsep, filedg,
  glob_topform, name_edit;
static char saved_sname[256]={0}, saved_dname[256]={0};
static Tsd_t *saved_sgoal=NULL, *saved_dgoal=NULL, *saved_ssgoal=NULL, *saved_dsgoal=NULL;
static Boolean_t confirm_import;
static int import_rec_num;
static char today_str[16];
...
printf("i_f 1\n");
imported_flag=FALSE;
saved_sname[0]=saved_dname[0]=0;
if (saved_sgoal != NULL)
{
  Tsd_Destroy (saved_sgoal);
  saved_sgoal = NULL;
}
if (saved_dgoal != NULL)
{
  Tsd_Destroy (saved_dgoal);
  saved_dgoal = NULL;
}
if (saved_ssgoal != NULL)
{
  Tsd_Destroy (saved_ssgoal);
  saved_ssgoal = NULL;
}
if (saved_dsgoal != NULL)
{
  Tsd_Destroy (saved_dsgoal);
  saved_dsgoal=NULL;
}
...
*/
  filedg = XmCreateFileSelectionDialog (top_level, "SchImportFSD", NULL, 0);
  XtVaSetValues (filedg, XmNdialogStyle, XmDIALOG_FULL_APPLICATION_MODAL, NULL);

  topform /*[which]*/ = XmCreateFormDialog (top_level, "Schema Edit Form", NULL, 0);
  glob_topform = topform;
...
*/
  ...
  strcpy (itemstr, "Import Schema Record");
  label = XmStringCreateLocalized (itemstr);

  imppb = XtVaCreateManagedWidget (itemstr,
  ...
  XtAddCallback (imppb, XmNactivateCallback, SchImport_CB, (XtPointer) NULL);

  impsep = XtVaCreateManagedWidget (itemstr,
...
XtSetArg(al[8],
    XmNvalue, schbuf[1]); ac++;
XtSetArg(al[11],
    XmNtitle,"Pretransform Tests"); ac++;
...
void SaveSch_Cont ()
{
  ...
  char modification[128], compare_date[128];
  ...
  if (new_schema_local || new_schema_open)
    ...
    sprintf (compare_date, "\007Schema modified by %s on %s at ", UserId (), today_str);
    modify_string = String_Create ((const char *) modification, 0);
    if (name_modified) was_modified[NAMMOD] = TRUE;
    if (eyc_modified) was_modified[EYCMOD] = TRUE;
    if (flags_modified) was_modified[FLGMOD] = TRUE;
    if (typmol_modified) was_modified[TYMMOD] = TRUE;
    if (refs_modified) was_modified[REFMOD] = TRUE;
    if (pretests_modified) was_modified[PREMOD] = TRUE;
    if (posttests_modified) was_modified[POSMOD] = TRUE;
    if (transform_modified) was_modified[XFMMOD] = TRUE;
/*
    if (schema_open)
      {
      nrefs = React_TxtHd_NumReferences_Get (txt_head);
...
*/
      ...
      nrefs = React_TxtHd_NumReferences_Get (txt_head);
      for (i = nrefs - 1;
        i >= 0 && strncmp (String_Value_Get (React_TxtRec_Reference_Get (text, i)), "\007Schema modified by ", 20);
        i--)
printf("checking %d: %s\n", i, (char *)String_Value_Get(React_TxtRec_Reference_Get(text,i)));
printf("i=%d: %s\n", i, i==0?"":(char *) String_Value_Get (React_TxtRec_Reference_Get (text, i)));
      if (i >= 0 && !strncmp (compare_date, String_Value_Get (React_TxtRec_Reference_Get (text, i)), strlen (compare_date)))
        {
        extract_flags (React_TxtRec_Reference_Get (text, i));
        append_flags (&modify_string);
        String_Destroy (React_TxtRec_Reference_Get (text, i));
        React_TxtRec_Reference_Put (text, i, modify_string);
        }
      else
...
printf("i_f 10: %d\n",!tests_changed);
  if (tests_changed) imported_flag=FALSE;
...
void SchImport_CB (Widget w, XtPointer client_data, XtPointer call_data)
{
  if (!XtIsManaged (glob_topform)) XtManageChild (glob_topform);
  if (logged_in)
        {
        if (clearance_level < (new_schema_local ? ADD_SCH_LEV : EDIT_TRANSFORM_LEV))
                {
                LoginFail (tl);
                return;
                }
        ...
        {
        XtUnmanageChild (topform);
        Login (tl, SchImport_Cont, topform);
        }
}
...
{
  Widget tf;
  XmString title, dir, ext, files, selection;
  char extstr[10];

/*
  printf ("calling SchImport_CB()\n");
*/
  if (!filedg_initd)
  {
    title = XmStringCreateLtoR ("Import Schema Data from ASCII Dump", XmFONTLIST_DEFAULT_TAG);
    dir = XmStringCreateLtoR (concat (FCB_SEQDIR_SYS (""), "/import"), XmFONTLIST_DEFAULT_TAG);
    sprintf (extstr, "*.%03d", sch_syntheme);
    ext = XmStringCreateLtoR (extstr, XmFONTLIST_DEFAULT_TAG);
    files = XmStringCreateLtoR ("Files", XmFONTLIST_DEFAULT_TAG);
    selection = XmStringCreateLtoR ("Selection", XmFONTLIST_DEFAULT_TAG);

    XtVaSetValues (filedg,
    ...
    XtAddCallback (filedg, XmNcancelCallback, SchImport_Cancel_CB, (XtPointer) NULL);

    XtAddCallback (filedg, XmNokCallback, SchImport_OK_CB, (XtPointer) NULL);
  ...
  XtManageChild (filedg);
}

void SchImport_Cancel_CB (Widget w, XtPointer client_data, XtPointer call_data)
{
  XtUnmanageChild (filedg);
}


void SchImport_OK_CB (Widget w, XtPointer client_data, XtPointer call_data)
{
  XmFileSelectionBoxCallbackStruct *filecb;
  char          *filename, warning[100], *buffer, itemstr[100], import_str[128];
  int size, nptests, lib_num, chap_num, sch_num, nrefs, hour, minute, now, i, sch_nums[2];
  FILE *f;
        React_Record_t *schema, *schemata[2];
        React_Head_t *sch_head;
        React_TextRec_t *text;
        React_TextHead_t *txt_head;
XmString label;
String_t import_string;
  Date_t today;
  ...
  filecb = (XmFileSelectionBoxCallbackStruct *) call_data;
  XtUnmanageChild (filedg);
  if (!XmStringGetLtoR (filecb->value, XmFONTLIST_DEFAULT_TAG, &filename)) return;
  schema=React_Schema_Handle_Get(React_NumSchemas_Get());
  sch_head=React_Head_Get(schema);
  text=React_Text_Get(schema);
  txt_head=React_TxtRec_Head_Get(text);
  lib_num = React_Head_Library_Get (sch_head);
  chap_num = React_Head_SynthemeFG_Get (sch_head);
  sch_num = React_TxtHd_OrigSchema_Get(txt_head);

  f=fopen(filename, "r");
  if (!f)
  {
    sprintf (warning, "Unable to open %s", filename);
    InfoWarn_Show (warning);
    return;
  }
  fseek (f, 0L, SEEK_END);
  size=ftell(f);
...
printf("%d\n",React_NumSchemas_Get());
  import_rec_num = React_NumSchemas_Get ();
  if (new_schema_local) React_Schema_Free (import_rec_num);
  else import_rec_num++;
printf("import_rec_num=%d\n",import_rec_num);
printf("&buffer=%p\n",&buffer);
  if (!React_Schema_Import(import_rec_num,buffer))
  {
    InfoWarn_Show ("import failed");
    return;
  }
printf("after R_S_I\n");
printf("&buffer=%p\n",&buffer);
...
if (new_schema_local)
{
  pretests_modified = posttests_modified = transform_modified = eyc_modified = typmol_modified = refs_modified = flags_modified =
    name_modified = FALSE;

    nptests = Post_Test_Count (import_rec_num, FALSE);
printf("nptests=%d\n",nptests);

    schema=React_Schema_Handle_Get(import_rec_num);
    sch_head=React_Head_Get(schema);
    text=React_Text_Get(schema);
    txt_head=React_TxtRec_Head_Get(text);
printf("got schema data\n");
    nrefs = React_TxtHd_NumReferences_Get (txt_head) + 1;
    React_TxtHd_NumReferences_Put (txt_head, nrefs);
    text->lit_refs = (String_t *) realloc (text->lit_refs, nrefs * sizeof (String_t));
    sprintf(import_str,"\007New schema imported by %s on %s at %02d%02d", UserId (), today_str, hour, minute);
    import_string = String_Create ((const char *) import_str, 0);
    React_TxtRec_Reference_Put (text, nrefs - 1, import_string);

    label = XmStringCreateLocalized ("Schema Editor");

    XtVaSetValues (glob_topform,
      XmNdialogTitle, label,
      NULL);
    XmStringFree (label);
printf("updated topform title\n");

    label = XmStringCreateLocalized ("Edit Pretransform Tests");
    XtVaSetValues (schpb[NUM_BUTTONS_PER_ROW],
      XmNlabelString, label,
      NULL);
    XmStringFree (label);
printf("updated pretran button\n");
    ...
    XtVaSetValues (schpb[2 * NUM_BUTTONS_PER_ROW],
      ...
      NULL);
    XmStringFree (label);
printf("updated postran button\n");

    label = XmStringCreateLocalized ("Edit Transform");
    XtVaSetValues (schpb[2],
      XmNlabelString, label,
      NULL);
    XmStringFree (label);
printf("updated transform button\n");

    XtRemoveAllCallbacks (name_edit, XmNmodifyVerifyCallback);
    XtRemoveAllCallbacks (name_edit, XmNvalueChangedCallback);

    XtVaSetValues (name_edit,
      XmNvalue, (String) String_Value_Get (React_TxtRec_Name_Get(text)),
      NULL);
    ...
    XtAddCallback (name_edit, XmNvalueChangedCallback, SchName_CB,
      (XtPointer) TRUE);
printf("updated name_edit field\n");
}
else
{
sch_nums[0]=import_rec_num-1;
sch_nums[1]=import_rec_num;
schemata[0]=schema;
schemata[1]=React_Schema_Handle_Get(import_rec_num);
SchComp_Create_Form (tl, schlC, sch_nums, schemata, &confirm_import, Check_Import);
/**********************************************************************************
Commented out on 4/5/02 - will eventually be part of a separate callback, due to
the need to compare the schemata prior to updating the record

    if (schemata_compatible (import_rec_num - 1, import_rec_num))
    {
printf("before R_S_F\n");
      React_Schema_Free (import_rec_num - 1);
printf("after R_S_F\n");
      React_Schema_Copy (import_rec_num, import_rec_num - 1);
printf("after R_S_C\n");
      nptests = Post_Test_Count (import_rec_num - 1, FALSE);
printf("after P_T_C\n");
    }
printf("before R_S_F\n");
    React_Schema_Free(import_rec_num);
    import_rec_num--;
    schema=React_Schema_Handle_Get(import_rec_num);
    sch_head=React_Head_Get(schema);
    text=React_Text_Get(schema);
...
printf("after R_S_F\n");
    ...
    React_TxtHd_NumReferences_Put (txt_head, nrefs);
    text->lit_refs = (String_t *) realloc (text->lit_refs, nrefs * sizeof (String_t));
    sprintf(import_str,"\007Schema update imported by %s on %s at %02d%02d", UserId (), today_str, hour, minute);
    import_string = String_Create ((const char *) import_str, 0);
    React_TxtRec_Reference_Put (text, nrefs - 1, import_string);
************************************************************************************/
return; /* see above comment - this is also provisional */
}
    React_Head_Library_Put (sch_head, lib_num);
    React_Head_SynthemeFG_Put (sch_head, chap_num);
    React_TxtHd_OrigSchema_Put(txt_head, sch_num);
    React_HeadFlags_Incomplete_Put(sch_head, TRUE);
imported_flag=TRUE;
printf("g_t13\n");
    get_text(import_rec_num,TRUE);
printf("got text\n");
    get_tests(import_rec_num,TRUE);
printf("got tests\n");
    get_patts(import_rec_num,TRUE);
printf("got patts\n");
printf("v_e11\n");
val_enable(FALSE,TRUE);
exit_enable(TRUE);
...
Boolean_t schemata_compatible (int dest, int src)
{
  React_Record_t *schema[2];
  React_Head_t *sch_head[2];
  React_TextRec_t *text[2];
  React_TextHead_t *txt_head[2];
  int sch_inx[2], i, chap_num[2], roots[MX_ROOTS], synthemes[MX_ROOTS], found, tr, ts, nameval, pattval;
  char warning[100];
  ...
  for (i=0; i<2; i++)
  {
    schema[i]=React_Schema_Handle_Get(sch_inx[i]);
    sch_head[i]=React_Head_Get(schema[i]);
    text[i]=React_Text_Get(schema[i]);
    txt_head[i]=React_TxtRec_Head_Get(text[i]);
    chap_num[i]=React_Head_SynthemeFG_Get (sch_head[i]);
  }
  if (chap_num[0] != chap_num[1])
  {
    for (i=0, found=-1; i<MX_ROOTS; i++)
    {
      roots[i]=React_Head_RootNode_Get(sch_head[1], i);
      synthemes[i]=React_Head_RootSyntheme_Get(sch_head[1], i);
      if (synthemes[i]==chap_num[0]) found=i;
    }
    if (found < 0)
    {
      sprintf (warning, "No root found corresponding to syntheme %d.", chap_num[0]);
      InfoWarn_Show (warning);
      saved_sname[0]=saved_dname[0]=0;
if (saved_sgoal != NULL)
{
...
if (saved_dsgoal != NULL)
{
  ...
  saved_dsgoal=NULL;
    ...
    if (found > 0)
    {
      ...
      ts=synthemes[0];
      roots[0]=roots[found];
      synthemes[0]=synthemes[found];
      roots[found]=tr;
      synthemes[found]=ts;
      for (i=0; i<MX_ROOTS; i++)
      {
        React_Head_RootNode_Put(sch_head[1], i, roots[i]);
        React_Head_RootSyntheme_Put(sch_head[1], i, synthemes[i]);
      }
    }
  }
  nameval=names_similar (String_Value_Get (React_TxtRec_Name_Get (text[0])), String_Value_Get (React_TxtRec_Name_Get (text[1])));
  pattval=(patterns_similar (React_Goal_Get(schema[0]), React_Goal_Get(schema[1])) +
    patterns_similar (React_Subgoal_Get(schema[0]), React_Subgoal_Get(schema[1])))/2;
  if ((nameval==100 && pattval>50) || (nameval>50 && pattval==100))
  {
    saved_sname[0]=saved_dname[0]=0;
if (saved_sgoal != NULL)
{
...
if (saved_dsgoal != NULL)
{
  ...
  if (pattval==100)
  {
    if (!strcmp(String_Value_Get (React_TxtRec_Name_Get (text[0])), saved_sname)
      && !strcmp(String_Value_Get (React_TxtRec_Name_Get (text[1])), saved_dname))
    {
      saved_sname[0]=saved_dname[0]=0;
if (saved_sgoal != NULL)
{
...
if (saved_dsgoal != NULL)
{
  ...
  saved_dsgoal=NULL;
    ...
    sprintf (warning, "Names differ:\n  %s\n    vs\n%s\n(If you want to override this, repeat the import.)",
      String_Value_Get (React_TxtRec_Name_Get (text[0])), String_Value_Get (React_TxtRec_Name_Get (text[1])));
    InfoWarn_Show (warning);
    strcpy (saved_sname, String_Value_Get (React_TxtRec_Name_Get (text[0])));
    strcpy (saved_dname, String_Value_Get (React_TxtRec_Name_Get (text[1])));
    return (FALSE);
  }
  if (nameval==100)
  {
    if (patterns_similar (React_Goal_Get(schema[0]), saved_sgoal)==100 &&
      patterns_similar (React_Goal_Get(schema[1]), saved_dgoal)==100 &&
      patterns_similar (React_Subgoal_Get(schema[0]), saved_ssgoal)==100 &&
      patterns_similar (React_Subgoal_Get(schema[1]), saved_dsgoal)==100)
    {
      Tsd_Destroy (saved_sgoal);
      Tsd_Destroy (saved_dgoal);
      Tsd_Destroy (saved_ssgoal);
      Tsd_Destroy (saved_dsgoal);
      saved_sgoal=saved_dgoal=saved_ssgoal=saved_dsgoal=NULL;
      return (TRUE);
    }
    InfoWarn_Show ("Patterns differ.\n(If you want to override this, repeat the import.)");
    saved_sgoal = Tsd_Copy (React_Goal_Get(schema[0]));
    saved_dgoal = Tsd_Copy (React_Goal_Get(schema[1]));
    saved_ssgoal = Tsd_Copy (React_Subgoal_Get(schema[0]));
    saved_dsgoal = Tsd_Copy (React_Subgoal_Get(schema[1]));
    return (FALSE);
  }
}

int names_similar (char *sname, char *dname)
{
  char *shorter, *longer;
  int i, j;

  if (!strcmp(sname,dname)) return(100);
  for (i=1; i<=strlen(sname) && i<=strlen(dname); i++) if (!strncmp(sname, dname, i))
  {
    if (i>=3*strlen(sname)/4 && i>=3*strlen(dname)/4) return(51);
  }
  for (i=1; i<=strlen(sname) && i<=strlen(dname); i++) if (!strncmp(sname+strlen(sname)-i,dname+strlen(dname)-1,i))
  {
    if (i>=3*strlen(sname)/4 && i>=3*strlen(dname)/4) return(51);
  }
  if (strlen(sname) == strlen(dname)) return(1);
  ...
  if (strlen(longer) >= 11*strlen(shorter)/5) return(5);
  for (i=0; i<=strlen(longer)-strlen(shorter); i++) for (j=1; j<=strlen(shorter); j++) if (!strncmp(shorter,longer+i,j))
  {
    if (j>=3*strlen(shorter)/4) return(51);
  }
  return(1);
}

int patterns_similar (Tsd_t *spatt, Tsd_t *dpatt)
{
  return(100);
}
...
void Check_Import ()
{
  int nptests, nrefs, hour, minute;
        React_Record_t *schema, *schemata[2];
        React_Head_t *sch_head;
        React_TextRec_t *text;
        React_TextHead_t *txt_head;
String_t import_string;
char import_str[128], itemstr[64];
XmString label;
  Date_t today;
...
minute = now - 60 * hour;
  ...
  sprintf(import_str,"\007Schema imported by %s on %s at %02d%02d", UserId (), today_str, hour, minute);
  import_string = String_Create ((const char *) import_str, 0);
  React_TxtRec_Reference_Put (text, nrefs - 1, import_string);
React_HeadFlags_Incomplete_Put(sch_head, TRUE);
imported_flag=TRUE;
