...
Boolean_t first_solved_this_cycle, already_solved, do_not_touch_pst_flag;

  first_solved_this_cycle = already_solved = do_not_touch_pst_flag = FALSE;
...
/* get this data NOW - or it will be missing from the status file! */
      ...
      if (end_search && write_fail_str[0] == '\0')
        {
        RunStats_Flags_StatusSaved_Put (rstat_p, TRUE);
        RunStats_Flags_PstChanged_Put (rstat_p, FALSE);
        if (end_search) do_not_touch_pst_flag = TRUE;
          /* Do NOT set changed flag back to true before exit and screw up this logic!!! */
        }
