  ...
  )
{
  ...
  if (!dsp_Shelley (RxnV_IthM_Mol_Get (rxnv_p, 0)))
    {
    fprintf (stderr, 
      "\nRxnView_Update:  unable to calculate coords for product molecule.\n");
    free_Molecule (RxnV_IthM_Mol_Get (rxnv_p, 0));
    RxnV_IthM_Mol_Put (rxnv_p, 0, NULL);
    }
  else
  ...
  )
{
  ...
  U8_t             mol_i;
  Boolean_t        passed;
Zadehan_t fuzzy_pass, true, false, ambiguous;
Boolean_t fuzzy;
char testmess_fuzzyfail[100];

  if (ssv_p == NULL)
    return;

strcpy (testmess_fuzzyfail, SSV_TESTMESS_FAIL);
testmess_fuzzyfail[strlen(testmess_fuzzyfail)-1]='\0'; /* remove period */

Zadehan_Byteval (true) = 201; /* 100% confidence << 1 | TRUE */
Zadehan_Byteval (false) = 200; /* 100% confidence << 1 | FALSE */
Zadehan_Byteval (ambiguous) = 101; /* 50% confidence << 1 | TRUE */
  ...
  for (mol_i = 1; mol_i < RVW_MAXNUM_MOLS 
    ...
    if (!dsp_Shelley (RxnV_IthM_Mol_Get (rxnv_p, mol_i)))
      ...
      fprintf (stderr, 
        ...
        mol_i);
      free_Molecule (RxnV_IthM_Mol_Get (rxnv_p, mol_i));
      RxnV_IthM_Mol_Put (rxnv_p, mol_i, NULL);
      }
    else
...
printf("SSV_U4.3b11\n");
        ...
        (Post_Result_Get (post_p) && SShotInfo_IthTRslt_Get (ssi_p, test_i) >= ambiguous.fuzzy_value))
        {
#ifdef _DEBUG_
printf("SSV_U4.4b10\n");
#endif
        Zadehan_Byteval (fuzzy_pass) = SShotInfo_IthTRslt_Get (ssi_p, test_i);
        if (Zadehan_Byteval (fuzzy_pass) == SSV_PTEST_PASS)
          {
          Zadehan_Byteval (fuzzy_pass) = Zadehan_Byteval (true);
          fuzzy = FALSE;
          }
        else if (!fuzzy_pass.fuzzy_components.TF) 
          {
          fuzzy_pass.fuzzy_components.TF = !fuzzy_pass.fuzzy_components.TF;
          fuzzy_pass.fuzzy_components.conf = 100 - fuzzy_pass.fuzzy_components.conf;
          fuzzy = TRUE;
          }
        sprintf (tbuf_p, "%s%1hu:  %s (%u%%)", SSV_TESTNUM, test_i + 1, 
          SSV_TESTMESS_PASS, fuzzy_pass.fuzzy_components.conf);
        tbuf_p += strlen (tbuf_p); 
        if (Post_EaseAdj_Get (post_p) != 0)
          {
          ease_adj_p += (Post_EaseAdj_Get (post_p) * fuzzy_pass.fuzzy_components.conf) / 100;
          if (Post_EaseAdj_Get (post_p) < 0)
            {
            sprintf (tbuf_p, ", %s%1d", SSV_TESTMESS_EASEDEC, 
              -Post_EaseAdj_Get (post_p) * fuzzy_pass.fuzzy_components.conf / 100);
            tbuf_p += strlen (tbuf_p); 
            }
          else
            {
        ...
        if (Post_YieldAdj_Get (post_p) != 0)
          {
        yield_adj_p += (Post_YieldAdj_Get (post_p) * fuzzy_pass.fuzzy_components.conf) / 100;
          if (Post_YieldAdj_Get (post_p) < 0)
            {
            sprintf (tbuf_p, ", %s%1d", SSV_TESTMESS_YLDDEC, 
              -Post_YieldAdj_Get (post_p) * fuzzy_pass.fuzzy_components.conf / 100);
            tbuf_p += strlen (tbuf_p); 
            }
          else
            {
        ...
        if (Post_ConfidenceAdj_Get (post_p) != 0)
          {
          conf_adj_p += (Post_ConfidenceAdj_Get (post_p) * fuzzy_pass.fuzzy_components.conf) / 100;
          if (Post_ConfidenceAdj_Get (post_p) < 0)
            {
            sprintf (tbuf_p, ", %s%1d", SSV_TESTMESS_CONFDEC, 
              -Post_ConfidenceAdj_Get (post_p) * fuzzy_pass.fuzzy_components.conf / 100);
            tbuf_p += strlen (tbuf_p); 
            }
          else
            {
...
#endif
        Zadehan_Byteval (fuzzy_pass) = SShotInfo_IthTRslt_Get (ssi_p, test_i);
        if (Zadehan_Byteval (fuzzy_pass) == SSV_PTEST_FAIL)
          {
#ifdef _DEBUG_
printf("SSV_U4.5b10a0\n");
#endif
          Zadehan_Byteval (fuzzy_pass) = Zadehan_Byteval (false);
          fuzzy = FALSE;
#ifdef _DEBUG_
printf("SSV_U4.5b10a1\n");
#endif
          }
        else if (fuzzy_pass.fuzzy_components.TF) 
          {
#ifdef _DEBUG_
printf("SSV_U4.5b10b0\n");
#endif
          fuzzy_pass.fuzzy_components.TF = !fuzzy_pass.fuzzy_components.TF;
          fuzzy_pass.fuzzy_components.conf = 100 - fuzzy_pass.fuzzy_components.conf;
          fuzzy = TRUE;
#ifdef _DEBUG_
printf("SSV_U4.5b10b1\n");
#endif
          }
        if (fuzzy)
          {
          sprintf (tbuf_p, "%s%1hu:  %s (%u%%), %s%1d, %s%1d, %s%1d\n     %s\n\n", SSV_TESTNUM, test_i + 1,
            testmess_fuzzyfail, fuzzy_pass.fuzzy_components.conf,
            SSV_TESTMESS_EASEDEC, (React_Head_Ease_Get (rxn_head_p) + ease_adj_p) * fuzzy_pass.fuzzy_components.conf / 100,
            SSV_TESTMESS_YLDDEC, (React_Head_Yield_Get (rxn_head_p) + yield_adj_p) * fuzzy_pass.fuzzy_components.conf / 100,
            SSV_TESTMESS_CONFDEC, (React_Head_Confidence_Get (rxn_head_p) + conf_adj_p) * fuzzy_pass.fuzzy_components.conf / 100,
            (char *) String_Value_Get (React_TxtRec_Reason_Get (txt_rec_p, test_i)));
          ease_adj_p -= (React_Head_Ease_Get (rxn_head_p) + ease_adj_p) * fuzzy_pass.fuzzy_components.conf / 100;
          yield_adj_p -= (React_Head_Yield_Get (rxn_head_p) + yield_adj_p) * fuzzy_pass.fuzzy_components.conf / 100;
          conf_adj_p -= (React_Head_Confidence_Get (rxn_head_p) + conf_adj_p) * fuzzy_pass.fuzzy_components.conf / 100;
          }
        else sprintf (tbuf_p, "%s%1hu:  %s (%u%%)\n     %s\n\n", SSV_TESTNUM, 
          test_i + 1, SSV_TESTMESS_FAIL, fuzzy_pass.fuzzy_components.conf, (char *) 
          String_Value_Get (React_TxtRec_Reason_Get (txt_rec_p, test_i)));
        tbuf_p += strlen (tbuf_p); 
/*
        passed = FALSE;
