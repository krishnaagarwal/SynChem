...
static void          SElec (U8_t, float *, float *, Xtr_t *, Xtr_t *, 
                       Array_t *);
static S8_t          SElecwd (U8_t, Xtr_t *, Xtr_t *, Array_t *);
...
in_subgenr(-811212);
#endif
  while ( (i < num_of_posttests) && pass && !finish_test)
    {
...
in_subgenr(-811213);
    ...
    while (j < Post_Length_Get (post_p) && next_test == FALSE)
      ...
      if (insym < PT_TEST_ADD)                 /* C condition */
        ...
        stack[stack_top] = Array_1d8_Get (&condition_values, insym); 
        stack_top++;
      }
      ...
      else if (insym == BOOLOP_XOR)
      {
        ...
        insym -= PT_TEST_ADD;
         ...
         if (insym >= i)
           ...
           {
           stack[stack_top] = Array_1d1_Get (&test_values, insym); 
           stack_top++;
           }
        }
...
in_subgenr(-811214);
    ...
    if (next_test != TRUE && stack_top == 1)
      Array_1d1_Put (&test_values, i, stack[0]);
    else
      Array_1d1_Put (&test_values, i, FALSE);
    ...
    if (Array_1d1_Get (&test_values, i) == TRUE)
      ...
      if (Post_Result_Get (post_p) == FALSE)
        ...
        pass = FALSE;
        if (sshotinfo_p != NULL)
          SShotInfo_IthTRslt_Put (sshotinfo_p, i, SSV_PTEST_FAIL);
        }
      else
        {
        ...
        *conf_adj_p += Post_ConfidenceAdj_Get (post_p);
        if (sshotinfo_p != NULL)
          SShotInfo_IthTRslt_Put (sshotinfo_p, i, SSV_PTEST_PASS);
        }
  ...
  )
{
  ...
  result = SCompare_Two_Values (val1, val2, Cond_Op_Get (cond_p));
...
/* Fix annoying error where it doesn't matter! */
  ...
  switch (Cond_Type_Get (cond_p))
    {
    case PT_TYPE_ELECWD :
      result = SCond_Elecwd_Test (cond_p, xtr_p, goal_xtr_p, match_p);
      break;
    ...
    case PT_TYPE_BULKY :
      result = SCond_Bulky_Test (cond_p, xtr_p, goal_xtr_p, match_p);  
      break;
    ...
    case PT_TYPE_CARBONIUM :
      result = SCond_Carbonium_Test (cond_p, xtr_p, goal_xtr_p, match_p);
      break;

    case PT_TYPE_LVNGROUP :
      result = SCond_Lvngroup_Test (cond_p, dist_matrix_p, constant_p, 
        xtr_p, match_p);
      break;

    case PT_TYPE_MIGRATAP :
      result = SCond_Migratap_Test (cond_p, dist_matrix_p, constant_p, 
        xtr_p, match_p);
      break;
    ...
    case PT_TYPE_AROMSUB :
      result = SCond_Aromat_Sub_Test (cond_p, subgxtr_p, subgmap_p, match_p);
      break;
  ...
  )
{
  ...
  if (Cond_Base_Exists (cond_p) == TRUE)
    ...
    result = SCompare_Two_Values (SFdist (Cond_Path_Prime_Get (cond_p, 0),
      ...
      match_p), (S8_t) Cond_Base_Get (cond_p), Cond_Op_Get (cond_p));
    return (result);
    }
  ...
  )
{
  ...
  if (inductive + resonant >= 0.40) 
    return (2);
  else if (inductive + resonant >= 0.08)
    return (1);
  ...
  )
{
  ...
  if (count == 0 && ((sym == OP_AND && stack_top_value == FALSE) ||
      (sym == OP_OR && stack_top_value == TRUE)))
    {
    *op_p = index;
