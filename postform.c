...
*/
      ...
      dwform[i] = XtVaCreateManagedWidget ("TForm",
        ...
        XmNtopPosition, 400 * i,
        XmNtopAttachment, XmATTACH_POSITION,
        XmNbottomPosition, 400 * (i + 1),
        XmNbottomAttachment, XmATTACH_POSITION,
        XmNleftOffset, 0,
        XmNleftAttachment, XmATTACH_FORM,
        XmNrightOffset, 0,
...
void draw_pattern (Widget wg, Pixmap pm, int schn, int which, Boolean_t *numbered)
{
  ...
  if (gstsd == NULL)
    {
    XDrawString (disp,XtWindow(wg),gc,10,25,"<Undefined>",11);
    XDrawString (disp,pm,gc,10,25,"<Undefined>",11);
    return;
    }
...
*/
    ...
    mdraw2(dsp,pattsd,disp,XtWindow(wg),gc,25,50,275,200,TRUE,node_used,numbered);

    mdraw2(dsp,pattsd,disp,pm,gc,25,50,275,200,FALSE,node_used,numbered);
