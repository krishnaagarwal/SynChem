...
Boolean_t imported_flag = FALSE, was_modified[8] = {FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE};
...
*/
  for (i=0; i<8; i++) was_modified[i] = FALSE;
...
*/
                ...
                if ((!atom_p->isC && atom_conn[atm_i]) || s)
                {
                        ...
                        else if (!strcasecmp (atomsym, "#K")) strcpy (atomsym, "Ch");
                        else if (atomsym[0] == '$')
                          {
...
void move_schema_cont (Widget w, XtPointer client_data, XtPointer call_data)
{
  ...
  new_chap_num = (int)client_data /* adjust for new 1st button */ - 1;
...
void refresh_schema (Boolean_t save, Boolean_t close_schema, Boolean_t saved_elsewhere)
{
  ...
  char modification[128], today_str[16], compare_date[128];
  Boolean_t write_ref;
...
*/
  ...
  if (!new_schema_flag)
    ...
    /* update creation date to retain synchronization between date_last_mod and time_last_mod in persist index */
      ...
      sprintf (compare_date, "\007Schema modified by %s on %s ", UserId (), today_str);
      modify_string = String_Create ((const char *) modification, 0);
      if (last_schema_closed)
        ...
        if (!imported_flag)
        {
          nrefs = React_TxtHd_NumReferences_Get (txt_head);
          for (i=nrefs-1;
            i>=0 && strncmp (String_Value_Get (React_TxtRec_Reference_Get (text, i)), compare_date, strlen (compare_date));
            i--);
          if (i>=0)
            {
...
void extract_flags (String_t str)
{
  char *pos;

  pos = strstr (String_Value_Get (str), ":");
  if (!pos) return;
printf("pos: \"%s\"\n", pos);
  if (strstr (pos, "name")) {was_modified[NAMMOD] = TRUE; printf ("NAMMOD\n");}
  if (strstr (pos, "ease/yield/confidence")) {was_modified[EYCMOD] = TRUE; printf ("EYCMOD\n");}
  if (strstr (pos, "flags")) {was_modified[FLGMOD] = TRUE; printf ("FLGMOD\n");}
  if (strstr (pos, "type/molecularity")) {was_modified[TYMMOD] = TRUE; printf ("TYMMOD\n");}
  if (strstr (pos, "comments/references")) {was_modified[REFMOD] = TRUE; printf ("REFMOD\n");}
  if (strstr (pos, "pre")) {was_modified[PREMOD] = TRUE; printf ("PREMOD\n");}
  if (strstr (pos, "posttransform")) {was_modified[POSMOD] = TRUE; printf ("POSMOD\n");}
  if (strstr (pos, "reaction transform")) {was_modified[XFMMOD] = TRUE; printf ("XFMMOD\n");}
}

void append_flags (String_t *str)
{
  int i;
  Boolean_t list_empty, pretran;

  list_empty = TRUE;
  for (i=0; i<8; i++) if (was_modified[i])
  {
printf("was_modified[%d]=%d\n", i, was_modified[i]);
    if (!list_empty && !pretran) String_Concat_c (str, ", ");
    else
    {
      pretran = FALSE;
      if (list_empty) String_Concat_c (str, ": ");
    }
    switch (i)
    {
    case NAMMOD:
printf("NAMMOD\n");
      String_Concat_c (str, "name");
      break;
    case EYCMOD:
printf("EYCMOD\n");
      String_Concat_c (str, "ease/yield/confidence");
      break;
    case FLGMOD:
printf("FLGMOD\n");
      String_Concat_c (str, "flags");
      break;
    case TYMMOD:
printf("TYMMOD\n");
      String_Concat_c (str, "type/molecularity");
      break;
    case REFMOD:
printf("REFMOD\n");
      String_Concat_c (str, "comments/references");
      break;
    case PREMOD:
printf("PREMOD\n");
      if (was_modified[POSMOD])
      {
        pretran = TRUE; /* no comma after this */
        String_Concat_c (str, "pre- and ");
      }
      else String_Concat_c (str, "pretransform tests");
      break;
    case POSMOD:
printf("POSMOD\n");
      String_Concat_c (str, "posttransform tests");
      break;
    case XFMMOD:
printf("XFMMOD\n");
      String_Concat_c (str, "reaction transform");
      break;
    default:
printf("default: %d\n", i);
      break;
